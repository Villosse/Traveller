...
void drawGraph(Line *line, sf::RenderWindow *window) {
  ...
  if (line->len > 2) {
    for (size_t i = 1; i < line->len; i++) {
      ...
      sf::Vertex l[] = {sf::Vertex(p1, sf::Color::Black),
                        sf::Vertex(p2, sf::Color::Black)};
      window->draw(l, 2, sf::Lines);
    }
    sf::Vector2f p1(line->node_list[0]->x, line->node_list[0]->y);
    sf::Vector2f p2(line->node_list[line->len - 1]->x,
                    line->node_list[line->len - 1]->y);
    sf::Vertex l[] = {sf::Vertex(p1, sf::Color::Black),
                      sf::Vertex(p2, sf::Color::Black)};
    window->draw(l, 2, sf::Lines);
  }
...
void drawWindow(sf::RenderWindow *window, Line *line,
...
// Line* find_shortest_path(size_t nbGen, Genetic_algorithm* ga, Line* line,
...
Line *find_shortest_path(size_t nbGen, Genetic_algorithm *ga, Line *line,
  ...
  for (size_t i = 1; i < nbGen; i++) {
    ...
    size_t nb_mutation =
        (ga->population[0]->len - 1) * exp(-20 * percentage) + 1;
...
void init() {
  ...
  sf::RenderWindow window(screenMode, "Travelling Salesman Project",
                          sf::Style::Fullscreen);

  // Define the drawing area (everything except the bottom 50 pixels for
  // buttons)
  ...
  std::vector<std::string> buttonNames = {"Clear", "Next Step", "Final Result",
                                          "Distance : 0", "Quit"};
  std::vector<sf::Color> buttonColors = {
      sf::Color(230, 0, 108), sf::Color(167, 200, 14), sf::Color(0, 175, 212),
      sf::Color(221, 55, 55), sf::Color(245, 156, 46), sf::Color(47, 85, 117)};
  for (int i = 0; i < 5; ++i) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    buttonTexts[i].setOrigin(buttonTexts[i].getLocalBounds().width / 2,
                             buttonTexts[i].getLocalBounds().height / 2);
    buttonTexts[i].setPosition(buttons[i].getPosition() +
                               sf::Vector2f(buttonWidth / 2, buttonHeight / 2));
  }
  ...
  while (window.isOpen()) {
    ...
    while (window.pollEvent(event)) {
      ...
      // Mouse button pressed: add point if within draw area or check button
      // click
      if (event.type == sf::Event::MouseButtonPressed) {
        ...
        if (event.mouseButton.button == sf::Mouse::Left) {
          ...
          if (drawArea.contains(mousePos)) {
            ...
            if (points.size() > 2) {
              buttonNames[3] =
                  "Distance : " + std::to_string(line->total_distance);
            }
          } else {
            // Check button clicks
            for (int i = 0; i < 5; ++i) {
              if (isMouseOverButton(buttons[i], mousePos)) {
                if (i == 0) {
                  ...
                  line->create_loop();
                  ga = new Genetic_algorithm(line->node_list, line->len, 1000,
                                             4);
                  if (line->len > 2) {
                  ...
                  line->create_loop();
                  ga = new Genetic_algorithm(line->node_list, line->len, 10000,
                                             4);
