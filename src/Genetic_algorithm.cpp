...
// Arguments: _nodes is an array of Node pointers, _len_nodes is the number of
// nodes, _no_of_individuals is the number of individuals in the population,
// _nb_thread is the number of threads to use
Genetic_algorithm::Genetic_algorithm(Node **_nodes, size_t _len_nodes,
                                     size_t _no_of_individuals,
                                     size_t _nb_thread) {
  ...
  data =
      (struct Data_new_pop **)malloc(nb_thread * sizeof(struct Data_new_pop *));
...
// Assess the current generation and return the index of the best individual
// (the one with the shortest path) Returns: the index of the best individual
size_t Genetic_algorithm::generation_assessment() {
  ...
  for (size_t i = 1; i < no_of_individuals; i++) {
    if (population[i]->total_distance < population[best]->total_distance &&
        population[i]->total_distance > 0)
      best = i;
  }
  return best;
}
// Check if a node in the son's path exists in the given range of the father's
// path Arguments: indexSon is the index of the son in the population, genStart
// and genEnd are the range in the father's path, to_search is the index in the
// son's path to search for Returns: the index in the father's path if found,
// otherwise -1
int Genetic_algorithm::in_genFather(size_t indexSon, size_t genStart,
                                    size_t genEnd, size_t to_search) {
  for (size_t j = genStart; j < genEnd; j++)
    if (*(population[0]->node_list[j]) ==
        *(population[indexSon]->node_list[to_search]))
      return j;
  return -1;
}
...
void *pthread_create_new_population(void *argv) {
  ...
  for (size_t i = data->start; i < data->end; i++) {
    ...
    data->ga_instance->population[i]->node_list[len_minus_one] =
        data->ga_instance->population[i]->node_list[0];

    data->ga_instance->population[i]->total_distance -=
        data->ga_instance->population[i]->distance_between_node[len_minus_one];
    data->ga_instance->population[i]->distance_between_node[len_minus_one] =
        distance_between_node(
            data->ga_instance->population[i]->node_list[0],
            data->ga_instance->population[i]->node_list[len - 2]);
    data->ga_instance->population[i]->total_distance +=
        data->ga_instance->population[i]->distance_between_node[len_minus_one];
  }
  return NULL;
...
void Genetic_algorithm::create_new_population(size_t number_of_mutation) {
  ...
  for (size_t i = 0; i < nb_thread; i++) {
    pthread_create(&(thread_list[i]), nullptr, pthread_create_new_population,
                   data[i]);
  }
...
// Arguments: number_of_generation is the number of generations to run the
// algorithm Returns: a pointer to the Line object representing the shortest
// path found It isn't a new line, just a pointer to the best individual! It
// shoudn't be deleted!
Line *Genetic_algorithm::find_shortest_path(size_t number_of_generation) {
  for (size_t i = 1; i < number_of_generation; i++) {
    ...
    cout << "generation n°" << i << "\t\tnumber_of_mutation:" << nb_mutation
         << "\t\tpercentage:" << percentage * 100 << "%\r";
...
Genetic_algorithm::~Genetic_algorithm() {
  ...
  // I've no idea if it work well, but when I do it, this destructor doesn't
  // crash, so I suppose it is ok (and idc if not, bc it doesn't have a hudge
  // impact in the computer memory)
  delete[] nodes;
  delete[] population;
