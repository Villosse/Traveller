...
// Arguments: Node** _node_list (pointer to array of Node pointers), size_t _len
// (number of Nodes)
...
// Arguments: size_t number_of_point (number of nodes), size_t max (maximum
// coordinate value)
...
void Line::create_loop() {
  ...
  distance_between_node =
      (double *)realloc(distance_between_node, (len - 1) * sizeof(double));
...
// Arguments: size_t index1 (index of first node), size_t index2 (index of
// second node)
...
// Arguments: size_t index1 (index of first node), size_t index2 (index of
// second node) Returns: double (distance between the nodes)
...
// Equality operator: Checks if two Lines are equal based on their nodes and
// length
...
// Arguments: Node* p1 (pointer to first node), Node* p2 (pointer to second
// node) Returns: double (distance between the nodes)
